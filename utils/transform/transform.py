...
class GeoTransformation:
    ...
    def __init__(
            ...
            random_translate=False, min_image_overlap=0, min_bbox_overlap=0, min_bbox_count=0, mask_bbox_overlap=0,
        ...
        self.mask_bbox_overlap = max(mask_bbox_overlap or 0, 0)
    ...
    def __call__(self, image, word_bboxes=[], by_word_char_bboxes=[], masks=[], labels=[]):
        return self.crop_rotate_resize(image, word_bboxes=word_bboxes,
                                       by_word_char_bboxes=by_word_char_bboxes, masks=masks, labels=labels)
    ...
    def _get_located_patch_quad(self, ih, iw, patch_quad_rel, bboxes=[], labels=[]):
            ...
            if (self.min_bbox_count or not self.allow_partial_occurrence) and self.min_bbox_overlap:
                ...
                for index, bbox in enumerate(bboxes):
                    ...
                    if bbox_overlap >= self.min_bbox_overlap:
                        bbox_count += 1
                    if bbox_overlap <= self.mask_bbox_overlap:
                        new_labels[index] = 0
        ...
        if found_randomly:
            return patch_quad, trial_idx + 1, new_labels
        else:
            return None, trial_idx + 1, new_labels

    def crop_rotate_resize(self, image, word_bboxes=[], by_word_char_bboxes=[], masks=[], labels=[]):
        """
        Args:
            image (ndarray): (H, W, C) shaped ndarray.
            masks (List[ndarray]): List of (H, W) shaped ndarray the same size as the image.
        ...
        if not self.random_translate:
            ...
            patch_quad, num_trials, labels = self._get_located_patch_quad(ih, iw, patch_quad_rel,
                                                                          bboxes=word_bboxes, labels=labels)

        vflip, hflip = self.vflip and npr.randint(10) > 0, self.hflip and npr.randint(10) > 0
        ...
        return dict(found_randomly=found_randomly, num_trials=num_trials, matrix=M, labels=labels, **transformed)
...
class ComposedTransformation:
    def __init__(
        ...
        self.geo_transform_fn = GeoTransformation(
            ...
            min_image_overlap=min_image_overlap, min_bbox_overlap=min_bbox_overlap, mask_bbox_overlap=mask_bbox_overlap,
    ...
    def __call__(self, image, word_bboxes=[], by_word_char_bboxes=[], masks=[], labels=[], height_pad_to=None,
                 width_pad_to=None):
