...
from albumentations import transforms as tf
...
class CustomTransforms:
    def __call__(self, pil_img, annotations: dict):
        ...
        transform = A.Compose([
            tf.ColorJitter(brightness=0.5, contrast=0.5, saturation=0.5, hue=0.5,p=0.5),
            A.core.composition.OneOf(transforms=[
                tf.GaussNoise(p=1.0),
                tf.GaussianBlur(p=1.0),
                tf.Blur(p=1.0)
            ], p=0.1),
            A.core.composition.OneOf(transforms=[
                tf.RGBShift(p=1.0),
                tf.HueSaturationValue(p=1.0),
                tf.ChannelShuffle(p=1.0),
                tf.CLAHE(p=1.0),
                tf.RandomGamma(p=1.0),
                tf.RandomBrightnessContrast(p=1.0),
            ], p=0.1),
            A.core.composition.OneOf(transforms=[
                tf.RandomFog(p=1.0),
                tf.RandomRain(p=1.0),
